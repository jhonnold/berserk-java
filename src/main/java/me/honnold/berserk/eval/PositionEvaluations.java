...
public class PositionEvaluations {
    ...
    public int evaluatePawnStructure(Position position) {
        ...
        while (pawnBitboard != 0) {
            ...
            if (column > 0) isolated = ((columnMasks[column - 1] & originalPawnBitboard) == 0);
            if (column < 7)
    ...
    public int evaluateKnights(Position position) {
        ...
        while (knightsBB != 0) {
            ...
            while (knightAttacks != 0) {
                ...
                if ((masks.getPawnAttacks(position.sideToMove, attackedSquare) & opponentPawns) != 0)
                    pawnControlled++;
            }

            int deduction = 24 / squares;
    ...
    public int positionEvaluation(Position position) {
        ...
        score += this.evaluateKnights(position);
